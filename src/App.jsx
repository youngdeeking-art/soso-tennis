...
import { Trophy, Plus, Users, Calendar, Award, X, Trash2, ChevronLeft, ChevronRight, Check, Pencil, Lock, ChevronDown, ChevronUp, BarChart2, Crown, Filter } from 'lucide-react';
...
const QUARTERS = [
  { value: 1, label: '1분기', months: [1,2,3] },
  { value: 2, label: '2분기', months: [4,5,6] },
  { value: 3, label: '3분기', months: [7,8,9] },
  { value: 4, label: '4분기', months: [10,11,12] },
];
...
export default function App() {
  ...
  const getSortedFilteredStats = () => {
    ...
    else if (memberSort === 'type') result.sort((a, b) => {
      if (a.member_type === b.member_type) return a.name.localeCompare(b.name, 'ko');
      return a.member_type === 'regular' ? -1 : 1;
    });
    return result;
  };
                    ...
                    {hiddenCombos.length > 0 && (
                      <div className="bg-white rounded-lg border border-stone-200 overflow-hidden">
                        <div className="px-4 py-3 border-b border-stone-100 bg-yellow-50">
                          <h3 className="font-bold text-yellow-700 text-sm">📈 숨은 꿀조합</h3>
                        </div>
                        <div className="divide-y divide-stone-100">
            ...
            {officers.length > 0 && (
              <div className="bg-white rounded-lg border border-stone-200 overflow-hidden">
                <div className="px-4 py-3 border-b border-stone-100 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-stone-800">역대 회장</h3>
                </div>
                <div className="divide-y divide-stone-100">
                ...
                {sortedFilteredStats.map(member => (
                  <div key={member.id} className={`bg-white rounded-lg border p-4 flex items-center gap-3 ${getGenderBg(member.gender)}`}>
                    <div className="relative flex-shrink-0">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${member.gender==='M'?'bg-blue-100 text-blue-700':'bg-pink-100 text-pink-600'}`}>
      ...
      {showAddMatch && (
        <Modal onClose={() => { setShowAddMatch(false); setEditingMatch(null); }} title={editingMatch?'경기 수정':'경기 기록'}>
          ...
          <div className="flex gap-2 mt-5">
            ...
            <button onClick={saveMatch} disabled={!isValidMatch} className="flex-1 px-4 py-2.5 bg-emerald-800 text-white rounded-lg font-medium disabled:bg-stone-300">{editingMatch?'수정 완료':'저장'}</button>
          </div>
        </Modal>
      )}
    </div>
