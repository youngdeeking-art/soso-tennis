...
const QUARTERS = [{ value: 1, label: '1분기' },{ value: 2, label: '2분기' },{ value: 3, label: '3분기' },{ value: 4, label: '4분기' }];
...
export default function App() {
  ...
  const [dateGuests, setDateGuests] = useState({});
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [{ data: m }, { data: mt }, { data: att }, { data: off }, { data: gc }, { data: ac }] = await Promise.all([
  ...
  const deleteOfficer = async (id) => { if (!checkPassword()) return; await supabase.from('officers').delete().eq('id', id); await loadData(); };
  ...
  const getStats = () => members.map(member => {
    ...
    matches.filter(m=>!m.isScheduled).forEach(m => {
      ...
      if(isFo){if(draw)foDraws++;else if(won)foWins++;else foLosses++;}
      else{if(draw)baekDraws++;else if(won)baekWins++;else baekLosses++;}
    });
    ...
    const foTotal=foWins+foLosses+foDraws,baekTotal=baekWins+baekLosses+baekDraws;
  ...
  const renderCalBadges=(dateStr,isSmall=false)=>{
    ...
    const realMatches=dayMatches.filter(m=>!m.isScheduled);
    const scheduled=dayMatches.filter(m=>m.isScheduled);
    const allConfirmed=realMatches.length>0&&realMatches.every(m=>m.confirmed);
    return(
      <div className={`flex flex-col gap-0.5 ${isSmall?'absolute bottom-0.5 left-0.5 right-0.5':''}`}>
        ...
        {scheduled.length>0&&<CalBadge className="bg-orange-100 text-orange-700">📋{scheduled.length}</CalBadge>}
        {realMatches.length>0&&<CalBadge className={allConfirmed?'bg-blue-100 text-blue-800':'bg-yellow-100 text-yellow-800'}>{allConfirmed?<Lock size={7} className="mr-0.5"/>:'🎾'}{realMatches.length}</CalBadge>}
      </div>
    );
  };
                  ...
                  <div className="border border-stone-200 rounded-lg overflow-hidden">
                    ...
                    {attendanceOpen&&(
                      <div className="p-3 space-y-3">
                        ...
                        {attendanceDirty&&(
                          <button onClick={saveAttendance} className="w-full py-2 bg-emerald-700 text-white rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5">
                            <Save size={14}/> 참석자 저장
                          </button>
                        )}
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedDateMatches.filter(m=>!m.isScheduled).length>0&&(
                      isDateConfirmed?(
                        ...
                        <button onClick={()=>confirmDateMatches(selectedDate)} className="w-full py-2.5 bg-blue-600 text-white rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5"><Lock size={14}/> 경기 확정하기</button>
                      ):null
                    )}
                  </div>
                </div>
              ...
              <div className="bg-stone-50 rounded-lg p-3 text-sm text-stone-600 text-center">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-left text-xs">
                    <div><span className="text-stone-400 mr-1">포</span>{getMemberName(scoringMatch.teamA1,scoringMatch.date)}</div>
                    <div><span className="text-stone-400 mr-1">백</span>{getMemberName(scoringMatch.teamA2,scoringMatch.date)}</div>
                  </div>
                  <div className="text-stone-400 font-bold px-2">vs</div>
                  <div className="text-right text-xs">
                    <div><span className="text-stone-400 mr-1">포</span>{getMemberName(scoringMatch.teamB1,scoringMatch.date)}</div>
                    <div><span className="text-stone-400 mr-1">백</span>{getMemberName(scoringMatch.teamB2,scoringMatch.date)}</div>
                  </div>
                </div>
              </div>
      ...
      {showAddMatch&&(
        <Modal onClose={()=>{setShowAddMatch(false);setEditingMatch(null);}} title={editingMatch?'경기 수정':isScheduledMode?'대진 등록':'경기 기록'}>
          <div className="space-y-3">
            ...
            {!isScheduledMode&&(
              <button onClick={()=>setIsDraw(!isDraw)} className={`w-full py-2.5 rounded-lg border text-sm font-medium flex items-center justify-center gap-2 ${isDraw?'bg-yellow-100 border-yellow-400 text-yellow-800':'bg-white border-stone-300 text-stone-600'}`}>
                🤝 {isDraw?'무승부로 기록':'무승부인 경우 체크'}
              </button>
            )}
            {availablePlayers.length<4&&(
              <div className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">⚠️ 참석자가 부족해요. 캘린더에서 참석자를 먼저 체크해주세요.</div>
            )}
          </div>
...
function ComboCard({title,combos,emoji,valueColor,bgColor='bg-emerald-50',titleColor='text-emerald-800'}) {
  return(
    <div className="bg-white rounded-lg border border-stone-200 overflow-hidden">
      <div className={`px-4 py-3 border-b border-stone-100 ${bgColor}`}>
